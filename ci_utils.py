...
def enrich_line_with_date(line: str) -> str:
    return f"[{datetime.datetime.utcnow()}] {line}" if not line.startswith("[20") else line


def enrich_lines_with_date(lines: str) -> str:
    lines = lines.splitlines()
    return "\n".join([enrich_line_with_date(i) for i in lines if i.strip() != ""])
...
def run_and_yield(cmd, **kwargs):
    process = subprocess.Popen(
        cmd, **kwargs, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True
    )
